...
function GroomWearPage() {
  ...
  const collectData = async () => {
    try {
      const response = await fetch('http://localhost:4000/groomwear', {
        method: 'GET',
        headers: {
  ...
  const sendData = async () => {
    try {
      const response = await fetch('http://localhost:4000/filter_gromwear', {
        method: 'post',
  ...
  const searchData = async () => {
    console.log(search);
    try {
      const response = await fetch('http://localhost:4000/groomwearsearch', {
        method: 'post',
      ...
      <div className="container">
        <div className="row">
          {data.map((curElem) => {
            ...
            const path = `${curElem.gname}`;

            return (
              <div className="col-md-4" key={curElem.gid}>
                <NavLink to={path} className="link">
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      <h5 className="card-title">{curElem.gname}</h5>
                      <p className="address">{curElem.gdescripition}</p>
                      <p className="address">{curElem.gcity}</p>
                      <p className="type">{curElem.gtype}</p>
                      <p className="type">{curElem.gprice}</p>
